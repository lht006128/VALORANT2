...
uintptr_t getBaseAddress(uintptr_t pid) {
	...
	return Drive.GetModuleBase(pid, "VALORANT-Win64-Shipping.exe");
}

uintptr_t getBaseAddress2(uintptr_t pid) {

	// Add your memory r/w code here
	return Drive.GetModuleBase(pid, "stub.dll");
}
...
template <typename T> void write(uintptr_t pid, uintptr_t address, T& buffer,ULONG Size) {
	Drive.WritePtr(pid, address,buffer,Size);
	// Add your memory r/w code here
}
...
uintptr_t decryptWorld(uintptr_t pid, uintptr_t base_address) {
	...
	//const auto key2 = ReadAddressFromGame(key);
	std::cout << "key" << key;
	...
	const auto uworld_ptr = decrypt_uworld(key, (uintptr_t*)&state);
		...
		//std::cout << "uworld" << ReadAddressFromGame(ReadAddressFromGame(uworld_ptr));
	return read<uintptr_t>(pid, ReadAddressFromGame(uworld_ptr));
}
...
int main()
{
	...
	if (!g_pid) {
	 ...
	 hProcess = OpenProcess(PROCESS_ALL_ACCESS, NULL, g_pid);
	std::cout <<"handle" <<  hProcess;


	BOOL a=Drive.SetPid(g_pid);
	if (a)
	{
		std::cout << "打开驱动设备:%d\n"<< a;
		//system("pause");
	...
	g_stu_address = getBaseAddress2(g_pid);

	std::cout << "base" << g_stu_address;
	...
	//ReadAddressFromGame(1205515400154);
...
uint64_t ReadAddressFromGame(uint64_t AddressToRead)
{
	...
	if (!RandomPageLocation)
		while (true)
		{
			//MMEMORY_BASIC_INFORMATION Info;
			//Drive.QueryMemory(g_pid, RandomPageLocation, &Info);
			MEMORY_BASIC_INFORMATIONA Info = { 0 };
			Drive.QueryMemory(g_pid, RandomPageLocation, Info);
			...
			if (Info.AllocationProtect == PAGE_EXECUTE_READWRITE && Info.Protect == PAGE_EXECUTE_READWRITE && Info.State == MEM_COMMIT)
			{
				...
				chunk_t A;
				//ReadProcessMemory(hProcess, (void*)RandomPageLocation, &A, sizeof(A), 0);
				Drive.ReadPtr(g_pid, RandomPageLocation, &A, sizeof(A));
				bool Good = true;
	...
	MemeStub = scanPattern((std::uint8_t*)g_base_address,0xEEFFFFFFFFFFFF, (char*)"\x48\x8B\x01\xC3", (char*)"xxxx");
	if (!MemeStub)
		return 0;

	// .data function pointer which gets called every frame, censored ofc
	uint64_t DataPtr = g_base_address + 0xDEADBEEF;

	static uint64_t OrigAddr = 0;
	if (!OrigAddr)
		Drive.ReadPtr(g_pid, DataPtr, &OrigAddr, sizeof(OrigAddr));
		//ReadProcessMemory(hProcess, (void*)DataPtr, &OrigAddr, sizeof(OrigAddr), 0);
	...
	*(uint64_t*)(&ReadStub[0] + 46) = OrigAddr;									// Original Location
	...
	//WriteProcessMemory(hProcess, (void*)RandomPageLocation, &ReadStub[0], sizeof(ReadStub), 0);
	Drive.WritePtr(g_pid, RandomPageLocation, &ReadStub[0], sizeof(ReadStub));

	//WriteProcessMemory(hProcess, (void*)DataPtr, &RandomPageLocation, sizeof(DataPtr), 0);
	Drive.WritePtr(g_pid, DataPtr, &RandomPageLocation, sizeof(DataPtr));
	uint64_t ReadBuff = 0;

	while (true)
	{
		//ReadProcessMemory(hProcess, (void*)(RandomPageLocation + sizeof(ReadStub)), &ReadBuff, sizeof(ReadBuff), 0);
		Drive.ReadPtr(g_pid, (ULONG64)(RandomPageLocation + sizeof(ReadStub)), &ReadBuff, sizeof(ReadBuff));
		if (ReadBuff)
			break;
	}

	//WriteProcessMemory(hProcess, (void*)DataPtr, &OrigAddr, sizeof(OrigAddr), 0);
	Drive.WritePtr(g_pid, DataPtr, &OrigAddr, sizeof(OrigAddr));
	//WriteProcessMemory(hProcess, (void*)RandomPageLocation, &NullBuff[0], sizeof(NullBuff), 0);
	Drive.WritePtr(g_pid, RandomPageLocation, &NullBuff[0], sizeof(NullBuff));
